<script lang="ts">
  ...
  let calendarDays = $derived.by(() => {
    ...
    for (let i = 0; i < 42; i++) {
      ...
      days.push({
        ...
        isCurrentMonth: currentIter.getMonth() === viewMonth,
        isToday: currentIter.toDateString() === today.toDateString()
      });
  ...
  function changeMonth(step: number) {
    let targetMonth = viewMonth + step;
    ...
    if (targetYear < START_YEAR || (targetYear === START_YEAR && targetMonth < START_MONTH)) return;
    if (targetYear > END_YEAR || (targetYear === END_YEAR && targetMonth > END_MONTH)) return;
